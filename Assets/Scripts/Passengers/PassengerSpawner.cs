...
public sealed class PassengerSpawner : MonoBehaviour
{
    ...
    [Header("Anomaly identity copy")]
    [SerializeField, Range(0f, 1f)] private float anomalyCopyHumanIdentityChance = 0.55f;
    [SerializeField] private int recentHumanIdentityMemory = 12;
    [SerializeField] private bool highSkillAnomaliesUseRealCopiedCard = true;
    ...
    private readonly List<IdentitySnapshot> recentHumanIdentities = new();
    ...
    private void Awake()
    {
        ...
        if (decisionGate == null)
            decisionGate = FindFirstObjectByType<StopGate>();

        RebuildHumanIdentityPoolFromRegistry();
    }
    ...
    public void SpawnPassengers(int currentStopIndex, int stopCount)
    {
        ...
        for (int i = 0; i < spawnCount; i++)
        {
            ...
            TryApplyCopiedIdentity(p);

            var app = p.GetComponent<PassengerAppearance>();
            if (app != null)
            {
                int seed = StableHash(p.PassengerName) ^ Random.Range(int.MinValue, int.MaxValue);
                app.Randomize(seed);
            }
            ...
            int expectedFare = fareTable != null ? fareTable.GetFare(claimedStops) : claimedStops;
    ...
    private void TryApplyCopiedIdentity(Passenger p)
    {
        if (p == null || !p.IsAnomaly)
            return;

        if (recentHumanIdentities.Count == 0)
            RebuildHumanIdentityPoolFromRegistry();

        if (recentHumanIdentities.Count == 0)
            return;

        if (Random.value > anomalyCopyHumanIdentityChance)
            return;

        IdentitySnapshot source = recentHumanIdentities[Random.Range(0, recentHumanIdentities.Count)];
        p.SetVisibleIdentity(source.Name, source.DateOfBirth, source.IdNumber, source.ExpiryDate);

        AnomalyController anomaly = p.GetComponent<AnomalyController>();
        if (highSkillAnomaliesUseRealCopiedCard && anomaly != null && anomaly.Skill == AnomalySkill.High)
            p.SetIdVisual(PassengerIdVisual.Real);

        if (debugLogs)
            Debug.Log($"[Spawner] {p.PassengerName} copied visible ID {source.IdNumber}");
    }

    private void RememberHumanIdentity(Passenger p)
    {
        if (p == null || p.IsAnomaly)
            return;

        IdentitySnapshot snap = new IdentitySnapshot
        {
        ...
        if (string.IsNullOrWhiteSpace(snap.Name) || string.IsNullOrWhiteSpace(snap.IdNumber))
            return;

        recentHumanIdentities.Add(snap);
        while (recentHumanIdentities.Count > Mathf.Max(1, recentHumanIdentityMemory))
            recentHumanIdentities.RemoveAt(0);
    }

    private void RebuildHumanIdentityPoolFromRegistry()
    {
        recentHumanIdentities.Clear();

        foreach (Passenger p in PassengerRegistry.All)
        {
            if (p == null || p.IsAnomaly)
                continue;
    ...
    private Transform GetSpawnPointForStop(int stopIndex, int passengerNumberThisStop)
    {
        if (stopSpawnSets != null)
        {
            ...
            {
                StopSpawnSet set = stopSpawnSets[s];
