...
public class SeatManager : MonoBehaviour
{
    ...
    public bool TryTeleportToSeat(Passenger p, SeatAnchor targetSeat, bool forceSwap)
    {
        ...
        // Determine current seat (hierarchy first)
        var currentSeat = GetSeatForPassenger(p);

        // If we don't know current seat, allow a fallback (but DO NOT mark occupancy from this)
        if (currentSeat == null)
            currentSeat = FindNearestSeat(p.transform.position);
        ...
        if (targetSeat.Occupied)
        {
            ...
            // If we can't determine the current seat properly, swapping is unsafe.
            if (currentSeat == null) return false;

            // Swap: p -> target, other -> current
